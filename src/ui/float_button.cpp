...
FloatButton::FloatButton(const PixmapGroup& pixmapGroup, QWidget *parent): ElaIconButton(pixmapGroup.startLight, parent) {
    ...
    if (config.getBool(ConfigItem::IsFloatButtonLight)) {
        this->setPixmap(pixmapGroup.startLight);
    } else {
        this->setPixmap(pixmapGroup.startDark);
    }
    this->move(config.getPoint(ConfigItem::FloatButtonPos));
    this->setFixedSize(config.getSize(ConfigItem::FloatButtonSize));
    ...
    if (event->buttons() & Qt::LeftButton) {
        if (QApplication::keyboardModifiers() & Qt::ControlModifier) {
            ...
            if (QApplication::keyboardModifiers() & Qt::AltModifier) {
                this->setFixedSize(config.getSize(ConfigItem::FloatButtonOriginalSize));
            }
        }
            ...
            if (config.getBool(ConfigItem::IsFloatButtonLight)) {
                ...
                config.setBool(ConfigItem::IsFloatButtonLight, false);
            } else {
                if (this->isPack) {
                ...
                config.setBool(ConfigItem::IsFloatButtonLight, true);
            }
        }
...
void FloatButton::mouseMoveEvent(QMouseEvent *event) {
    if (event->buttons() & Qt::LeftButton) {
        if (QApplication::keyboardModifiers() & Qt::ControlModifier) {
            ...
            config.setPoint(ConfigItem::FloatButtonPos, event->globalPos() - this->dragPos);
    ...
    if (!numSteps.isNull()) {
        ...
        if (QApplication::keyboardModifiers() & Qt::ControlModifier) {
            if (scrollY > 0) {
                ...
                size *= 1.1;
                config.setSize(ConfigItem::FloatButtonSize, size);
