...
4. 优化了使用UV新建项目的体验
5. 重构打包，设置UI界面
6. 修改了软件的标题格式，让其可以根据NPF文件路径显示
7. 添加打包日志功能，数据存储在npf文件中
8. 添加最小化到系统托盘的功能
9. 添加了在打开软件时自动打开上一次打开的npf文件的功能
10. 修改了菜单栏中的"文件"菜单中的内容
11. 添加了防呆机制，如果不打开npf文件，就无法编辑
12. 为托盘栏添加了开始和停止打包的功能
13. 添加了在打包结束和被停止时显示系统通知的功能
...
代码方面的修改
重大修改：
1. 将项目配置的存储方式从struct改为了PCM

普通修改：
1. 添加了全局变量管理类GlobalData
2. 新增了types/simname.h文件，可以通过此文件定义一些常用数据的简写
3. 重构了Config类
4. 添加了GlobalDataItemName命名空间，别名为GDIN
5. 在GlobalData类中添加了信号valueChanged
*/
